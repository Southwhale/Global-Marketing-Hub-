...
import { X, Save, Plus, Trash2, Calendar, Users, DollarSign, MessageSquare, Bell, MapPin, Paperclip, FileText, TrendingUp, AlertTriangle } from 'lucide-react';
...
export const TaskModal: React.FC<TaskModalProps> = ({ task: initialTask, onClose }) => {
  const { addTask, updateTask, kpis } = useStore();
  ...
  const calculatedSpent = totalScheduleCost + totalRegionCost + (task.activityCost || 0) + (task.executionCost || 0);
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <TeamMemberSelect
                    label="Owners"
                    selectedIds={task.owner ? task.owner.split(',').map(s => s.trim()).filter(Boolean) : []}
                    onChange={(ids) => setTask({ ...task, owner: ids.join(', ') })}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-1">Activity Cost (활동비)</label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-1">Execution Cost (집행비)</label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input 
                          type="number" 
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-1">Target Revenue (목표 매출)</label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input 
                          type="number" 
                          value={task.targetRevenue || ''}
                          onChange={e => setTask({...task, targetRevenue: Number(e.target.value)})}
                          className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none font-medium"
                          placeholder="0"
                        />
                      </div>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-1">Actual Revenue (현황 수치)</label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input 
                          type="number" 
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                      <span className="text-sm font-bold text-slate-700">Calculated ROI</span>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => {
                      const region = prompt('Enter region name (ASIA, MENA, EU, CIS, LATAM, NA, OCEANIA, RUS, GLOBAL, KOREA, VINA):');
